...
@Tag(name = "Straightforward Expression Handler")
@RestController
...
public class StraightforwardExpressionApplication extends ExpressionHandler {
	...
	public ResponseEntity<?> processTask (String expression) throws Exception {
		try {
			...
			return ResponseEntity.ok(StraightforwardExpressionApplication.this.client.getForObject("http://invalid-expression:8105/assumeResponsibility/" + "{expression}", String.class, 
				   expression.replace("b", "")));
		}
    }

	StraightforwardExpressionApplication() {
		criteria = new char[]{'(', ')'};
		address_of_next_handler = "http://bodmas-expression:8103/assumeResponsibility/";
	}
    ...
    @Operation(summary = "Delegate or handle a straightforward mathematical expression")
	@GetMapping(address)
	...
	public Boolean considerDelegation(String expression) {
        if (expression.charAt(expression.length() - 1) == 'b') return false;
