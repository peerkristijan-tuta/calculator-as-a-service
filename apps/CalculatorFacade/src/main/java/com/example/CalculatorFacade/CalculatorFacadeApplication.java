...
public class CalculatorFacadeApplication {
	...
	String delegateToHandler(String expression) throws Exception {
        ...
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create("http://straightforward-expression:8102/assumeResponsibility/" + UriUtils.encodePath(expression, "UTF-8"))).build();
        return client.send(request, HttpResponse.BodyHandlers.ofString()).body();
	}

	@Operation(summary = "Retrieve up to 5 past computation records for a given identifier")
	@GetMapping("/retrieveRecord/{identifier}")
	public ResponseEntity<ArrayList<String[]>> retrieveRecord(@Parameter(description = "Session identifier of API Gateway") @PathVariable String identifier) {
	...
	@Operation(summary = "Submit a mathematical expression to be evaluated and logged")
	@GetMapping("/receiveExpression/{expression}/{identifier}")	
	public ResponseEntity<String> receiveExpression(@Parameter(description = "The expression to solve") @PathVariable String expression, 
													@Parameter(description = "The session identifier") @PathVariable String identifier) throws Exception {
